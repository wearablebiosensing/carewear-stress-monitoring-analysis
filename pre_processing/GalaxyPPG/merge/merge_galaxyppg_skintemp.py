...
def label_skintemp_data(skintemp_df, intervals_df):
    """
    Assign session labels and integer mappings to SkinTemp samples.
    Same logic as HR labeling, just applied to SkinTemp rows.
    """
    skintemp_df["label"] = "None"

    for _, row in intervals_df.iterrows():
        mask = (
            (skintemp_df["timestamp"] >= row["start_ts"]) &
            (skintemp_df["timestamp"] <= row["end_ts"])
        )
        skintemp_df.loc[mask, "label"] = row["session"]

    skintemp_df["activity_int_merged"] = (
        skintemp_df["label"].map(ACTIVITY_MAP).fillna(-1).astype(int)
    )
...
def process_participant(participant_path, output_dir):
    ...
    skintemp_file = os.path.join(participant_path, "GalaxyWatch", "SkinTemp.csv")

    if not os.path.exists(event_file) or not os.path.exists(skintemp_file):
    ...
    skintemp_df = pd.read_csv(skintemp_file)

    # Convert timestamps to numeric
    events_df["timestamp"] = pd.to_numeric(events_df["timestamp"], errors="coerce")
    skintemp_df["timestamp"] = pd.to_numeric(skintemp_df["timestamp"], errors="coerce")

    # Drop bad rows
    events_df.dropna(subset=["timestamp"], inplace=True)
    skintemp_df.dropna(subset=["timestamp"], inplace=True)
    ...
    # Ensure SkinTemp columns exist
    required_skintemp_cols = {"timestamp"}
    if not required_skintemp_cols.issubset(skintemp_df.columns):
        print(f"⚠️ {participant_id}: SkinTemp.csv missing required columns {required_skintemp_cols} — skipping.")
        return
    ...
    skintemp_labeled = label_skintemp_data(skintemp_df, intervals_df)

    skintemp_labeled["datetime"] = pd.to_datetime(
        skintemp_labeled["timestamp"], unit="ms", errors="coerce"
    )

    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(
        output_dir,
        f"{participant_id}_GalaxyWatch_SkinTemp_merged_with_events.csv"
    )
    skintemp_labeled.to_csv(output_file, index=False)

    print(f"  ✓ Saved → {output_file}")

def main():
    parser = argparse.ArgumentParser(
        description="Merge GalaxyPPG SkinTemp data with Event labels and Int Mappings."
    )
    parser.add_argument(
        "--data",
        required=True,
    ...
    parser.add_argument(
        "--out",
        required=True,
        help="Directory to save merged SkinTemp files"
    )
    parser.add_argument(
        "--pids",
        nargs="+",
